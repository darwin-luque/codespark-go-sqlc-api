...
func (fm *FavoritesModule) addArticle() http.HandlerFunc {
	...
	return http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		...
		favoriteData := domain.AddArticleAsFavoriteParams{
			ArticleID: article.ID,
			UserID:    user.ID,
		}

		favorite, err := q.AddArticleAsFavorite(req.Context(), favoriteData)
		if err != nil {
			utils.ServerError(w, err)
			return
		}

		utils.WriteJSON(w, http.StatusCreated, utils.M{"favorite": favorite})
	})
}
