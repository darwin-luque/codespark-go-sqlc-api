...
func (fm *FollowsModule) unfollowUser() http.HandlerFunc {
	q := domain.New(fm.db)
	...
	return func(w http.ResponseWriter, req *http.Request) {
		...
		unfollowUserInput := domain.UnfollowUserParams{
			FollowingUserID: user.ID,
			FollowedUserID:  userToFollow.ID,
		}

		if err := q.UnfollowUser(req.Context(), unfollowUserInput); err != nil {
			utils.ErrorResponse(w, http.StatusInternalServerError, err)
			return
		}
